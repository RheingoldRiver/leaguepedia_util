...
summary = 'LoL Pros full URL now' # Set summary
	...
	if lmt < startat:
		...
		for template in wikitext.filter_templates():
			if template.name.matches('Infobox Player'):
				if template.has('lolpros'):
					val = template.get('lolpros').value.strip()
					if not 'lolpros' in val:
						template.add('lolpros', 'https://lolpros.gg/player/' + val )
