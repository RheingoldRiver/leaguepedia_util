...
summary = 'server -> region'  # Set summary
	...
	if lmt < startat:
		...
		for template in wikitext.filter_templates():
			if template.name.matches('Infobox Tournament'):
				if template.has('server'):
					if (not template.has('region')) or template.get('region').value.strip() == '':
						s = template.get('server').value.strip()
						template.add('region', s)
						template.remove('server')
